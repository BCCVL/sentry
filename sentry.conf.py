...
postgres = (os.environ.get('SENTRY_POSTGRES_HOST') or
...
if postgres:
    DATABASES = {
        'default': {
            ...
            'NAME': (
                os.environ.get('SENTRY_DB_NAME') or
                os.environ.get('POSTGRES_ENV_POSTGRES_USER') or
                'sentry'
            ),
            'USER': (
                os.environ.get('SENTRY_DB_USER') or
                os.environ.get('POSTGRES_ENV_POSTGRES_USER') or
                'sentry'
            ),
            'PASSWORD': (
                os.environ.get('SENTRY_DB_PASSWORD') or
                os.environ.get('POSTGRES_ENV_POSTGRES_PASSWORD') or
                ''
            ),
            'HOST': postgres,
            'PORT': (
                os.environ.get('SENTRY_POSTGRES_PORT') or
                ''
            ),
            'OPTIONS': {
                'autocommit': True,
...
SENTRY_SINGLE_ORGANIZATION = Bool(os.environ.get('SENTRY_SINGLE_ORGANIZATION',
                                                 True))
...
redis = (os.environ.get('SENTRY_REDIS_HOST') or
...
memcached = (os.environ.get('SENTRY_MEMCACHED_HOST') or
             (os.environ.get('MEMCACHED_PORT_11211_TCP_ADDR') and 'memcached'))
if memcached:
    memcached_port = (
        os.environ.get('SENTRY_MEMCACHED_PORT') or
        '11211'
    )
    CACHES = {
        'default': {
...
# (read: if you send any kind of real data to Sentry, you should enable
#        buffers)
...
email = (os.environ.get('SENTRY_EMAIL_HOST') or
...
SENTRY_FEATURES['organizations:create'] = False
